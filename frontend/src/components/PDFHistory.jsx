...
export default function PDFHistory({ onClose }) {
  ...
  const [downloading, setDownloading] = useState('')  // "docId-formType"
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-secondary mt-0.5">
            PDFs are regenerated on download using your latest saved data
          </p>
        </div>
        <button
          onClick={onClose}
            ...
            {expanded[group.voucher_number] && (
              <div className="border-t border-border">
                {group.forms.map(form => {
                  const key = `${group.document_id}-${form.form_type}`
                    ...
                    <div key={form.form_type}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted mt-0.5">
                          Last generated: {new Date(form.generated_at).toLocaleString()}
                        </p>
                      </div>
                      <button
                        onClick={() => handleDownload(group.document_id, form.form_type, form.filename)}
                        disabled={isDown}
                        className="flex-shrink-0 flex items-center gap-1.5 text-xs text-accent hover:text-yellow-400 font-medium transition-colors px-2 py-1 rounded hover:bg-accent-bg disabled:opacity-40"
                      >
                        {isDown ? (
